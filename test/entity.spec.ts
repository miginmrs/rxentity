...
import { current } from '../source/valued-observable';
describe('Entity', () => {
  ...
  describe('Top Level Entity', () => {
    ...
    it('should ignore rewind, setParent, levelOf', () => {
      ...
      expect(userEntity.snapshot).deep.eq(userData2);
      expect(current(userEntity.levelOf('name'))).eq(0);
    })
    it('should have valued subjects as rx fields', () => {
      expect(current(userEntity.rx('name'))).eq(userData2.name);
  ...
  describe('toEntity', () => {
    it('should proxify fields', () => {
      expect(current(user.name)).eq(userData3.name);
  ...
  describe('Child Entity', () => {
    describe('Parent ready', () => {
      ...
      it('should implement levelOf to give the distance to the source subject', () => {
        expect(current(combineLatest([child.levelOf('name'), child.levelOf('login')]))).deep.eq([1, 0]);
      });
    ...
    describe('Parent not ready', () => {
      ...
      it('should have a consistent snapshot', () => {
        expect(current(child.name)).eq(childData.name);
        expect(current(child.firstName)).eq(childData.firstName);
        expect(child.snapshot).deep.eq(childData);
        child.rewind('name');
        expect(current(child.name)).deep.eq(childData.name);
      });
