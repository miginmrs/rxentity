...
describe('Store', () => {
  ...
  it('should implemenet changeId correctly', async () => {
    const stack: User[] = []
    const subscription = store.prepare(1n, () => Promise.reject(null)).subscribe(x=>stack.push(x.snapshot));
    const subject = new Subject<void>(), step1 = subject.pipe(take(1)).toPromise();
    const u = store.prepare(1n, (id) => {
      expect(id).eq(2n);
